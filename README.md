...
HUD와 메뉴는 `fonts/kbl/OTF/`의 KBL 로컬 폰트를 사용하며, 외부 웹 폰트 연결 없이 빌드에 포함됩니다. 폭탄 파괴와 플레이어 충돌에는 `폭탄터짐.mp3`와 강화된 파티클·점수 충격 연출이 함께 재생됩니다. 90초가 끝나는 정확한 종료 프레임에는 `타이머.mp3`가 한 번 재생됩니다.
